...
fn foreground_prompt() -> &'static str {
    "Session kind: foreground. You are interacting with the user directly. Prefer clear progress, \
     ...
     messages, use fenced code blocks with literal ``` only when showing real source code, exact \
     command snippets, raw logs, or structured data that must preserve formatting. Do not wrap \
     ordinary explanations, prose, tensor shapes, formulas, bullet lists, or status summaries in \
     ``` fences; prefer native Markdown paragraphs, lists, tables, and inline code for short \
     identifiers so responses stay human-readable. In final assistant \
...
mod tests {
    ...
    fn system_prompt_changes_by_session_type() {
        ...
        assert!(foreground.contains("literal ```"));
        assert!(foreground.contains("Do not wrap ordinary explanations"));
        assert!(foreground.contains("native Markdown paragraphs, lists"));
        ...
        assert!(!background.contains("literal ```"));
        assert!(!subagent.contains("literal ```"));

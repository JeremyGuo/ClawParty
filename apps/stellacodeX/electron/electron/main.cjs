...
function updateRemotePath(channel) {
  return `/home/liuhao/ClawParty/apps/stellacodeX/electron/release/${updateRemoteName(channel)}`;
}
...
function firstFileFromWorkspaceArchive(archive) {
...
async function previewWorkspaceFile(_event, payload) {
  const archive = await fetchWorkspaceDownloadArchive(payload);
  const first = firstFileFromWorkspaceArchive(archive);
  if (!first) throw new Error('文件为空或无法预览');
  const requestedLimit = Number(payload.maxBytes);
  const maxBytes = Number.isFinite(requestedLimit) && requestedLimit > 0
    ? Math.min(requestedLimit, WORKSPACE_PREVIEW_MAX_BYTES)
    : WORKSPACE_PREVIEW_MAX_BYTES;
  if (first.data.length > maxBytes) {
    throw new Error(`PDF 过大（${Math.ceil(first.data.length / 1024 / 1024)} MiB），请下载查看`);
  }
  const data = first.data.buffer.slice(
    first.data.byteOffset,
    first.data.byteOffset + first.data.byteLength
  );
  return {
    name: path.basename(first.name || payload.suggestedName || payload.path || 'preview.pdf'),
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('workspace:preview', previewWorkspaceFile);

...
use crate::config::{
    StellaclawConfig, CONFIG_VERSION_0_10, CONFIG_VERSION_0_11, CONFIG_VERSION_0_12,
    CONFIG_VERSION_0_13, CONFIG_VERSION_0_14, CONFIG_VERSION_0_2, CONFIG_VERSION_0_3,
...
pub fn load_config_file_and_upgrade(path: &Path) -> Result<(StellaclawConfig, bool)> {
    ...
    let mut config = match version.as_str() {
        ...
        CONFIG_VERSION_0_11 => v0_11::load(&raw, path)?,
        CONFIG_VERSION_0_12 => v0_12::load(&raw, path)?,
        CONFIG_VERSION_0_13 => v0_13::load(&raw, path)?,
        CONFIG_VERSION_0_14 => v0_14::load(&raw, path)?,
        LATEST_CONFIG_VERSION => v0_15::load(&raw, path)?,

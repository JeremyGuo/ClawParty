...
impl WorkdirUpgrader for Upgrade {
    ...
    fn upgrade(&self, workdir: &Path) -> Result<()> {
        backfill_session_actor_mailbox(&workdir.join("sessions"), "session.json")?;
        backfill_snapshot_actor_mailbox(&workdir.join("snapshots"), "snapshot.json")?;
        Ok(())
    }
}

fn backfill_session_actor_mailbox(root: &Path, file_name: &str) -> Result<()> {
    if !root.is_dir() {
        return Ok(());
    }
...
fn backfill_snapshot_actor_mailbox(root: &Path, file_name: &str) -> Result<()> {
    if !root.is_dir() {
        return Ok(());
    }
    ...
    for entry in fs::read_dir(root).with_context(|| format!("failed to read {}", root.display()))? {
        ...
        session.entry("actor_mailbox").or_insert_with(|| json!([]));

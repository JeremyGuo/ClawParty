...
fn build_static_intro_prompt_parts(
    ...
    vec![
        ...
        "Use update_plan for non-trivial, multi-step, ambiguous, or long-running work so the user can see your current checklist. Keep plan steps short, maintain at most one in_progress step, mark progress as it changes, and do not repeat the full plan in normal assistant text after updating it.".to_string(),
        ...
        "For repository exploration, use the dedicated tools instead of shell read/search commands: use glob to find files by path pattern, grep to find files by content pattern, ls for narrowed directory listings, and file_read for file contents. Do not call exec_start with direct grep, find, cat, head, tail, or ls; the runtime rejects those direct read/search commands. For remote work, use each tool's remote parameter instead of ssh plus shell text commands.".to_string(),
        ...
        "exec_start, exec_wait, and exec_observe cap returned stdout/stderr to at most 1000 characters. If you expect large output, set max_output_chars=0 and inspect the saved workspace-relative stdout_path and stderr_path with targeted dedicated tools or structured filters.".to_string(),
...
fn build_kind_static_prompt_parts(kind: AgentPromptKind) -> Vec<String> {
    ...
    match kind {
        AgentPromptKind::MainForeground => {
            ...
            parts.push("If you started a background agent and later decide to take over the task yourself, cancel or suppress that background agent before sending your own final answer. Its final reply will be inserted into the main foreground context, so do not let both the foreground turn and the background agent produce separate user-facing completions for the same task unless the user explicitly asked for multiple independent outputs.".to_string());
...
mod tests {
    ...
    fn prompt_includes_attachment_guidance_and_omits_channel_details() {
        ...
        assert!(prompt.contains("Do not call exec_start with direct grep"));
        assert!(prompt.contains("use each tool's remote parameter instead of ssh"));
